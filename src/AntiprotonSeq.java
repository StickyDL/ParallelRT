...
// Hidden variables.
    ...
    public static void simulate( long seed, int frames, int num_protons ){
    ...
    public static void simulate( long seed, double radius, double dt, int steps, int snapshots, int num_protons ){
        ...
        AntiprotonSeq.seed = seed;
        R = radius;
        AntiprotonSeq.dt = dt;
        AntiprotonSeq.steps = steps;
        snaps = snapshots;
        N = num_protons;
        ...
        for (int s = 0; s < snaps; ++ s)
            ...
            for( int k = 0; k < p.length; ++k )
            	{
            	points.add( new double[]{p[k].x, p[k].y} );
            	}
            }

        positions = points.toArray( new double[0][0] );
